...
export default async function PackageDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="section section-tint"><div className="shell split"><div><p className="eyebrow">{item.experienceLabel}</p><h2>Choose the expression,<br />then confirm the access.</h2><p className="lead">{item.reason}</p></div><div className="experience-choice-list">{item.experienceOptions.map((option, index) => <article key={option}><span>0{index + 1}</span><p>{option}</p></article>)}<p className="experience-choice-note">Examples are planning requests, not guaranteed inventory. Venue access, golf tee times, guides, vehicles, and every named experience require final confirmation.</p></div></div></section>
