...
export default async function PropertyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="section"><div className="shell"><div className="section-heading"><p className="eyebrow">Rooms</p><h2>Choose the space that fits your group.</h2><p>Room facts come from the current property content source and require a final publication review.</p></div><div className="room-table"><div className="room-header" aria-hidden="true"><span>Room type</span><span>Size</span><span>Capacity</span><span>Beds</span></div>{property.roomTypes.map((room) => <div className="room-row" key={room.name}><strong>{room.name}</strong><span data-label="Size">{room.size}</span><span data-label="Capacity">{room.guests}</span><span data-label="Beds">{room.beds ?? "Bed details to be confirmed"}</span></div>)}</div></div></section>
